...
export -f summarize
echo -e "TREE\tALN\tALNLEN\tmean_BSS\tmedian_BSS\tevoRate" > ../gene_trees.summarize_FT_JTT.tsv
parallel -j $CPU summarize ::: $(find . -name "*.${PEPTREEEXT}" ) >> ../gene_trees.summarize_FT_JTT.tsv
